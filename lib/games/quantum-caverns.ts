...
import { Coccoon, ImageList, Sprite, Text, SoundList, Sound, LOOP, color, Colors, type Game } from "@/lib/coccoon"
...
export class QuantumCaverns implements Game {
  ...
  ready(engine: Coccoon): void {
    ...
    // Ambient theme, generated by the Moth platform's retrocausal-echo-v1
    // engine: a short seed motif fed through a quantum-measured multi-tap delay,
    // whose taps invert/reverse/rotate the signal into an evolving cave drone.
    // Loops quietly under the game; audio unblocks on the player's first keypress
    // (browsers keep it suspended until a user gesture).
    new SoundList(engine, ["/audio/caverns-theme.wav"])
    this._music = new Sound(engine, 0, LOOP, 0.5)
